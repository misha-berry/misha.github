...
// For voltage divider to measure battery voltage
const float R1 = 10000.0;  // top resistor in ohms
const float R2 = 5100.0;   // bottom resistor in ohms
...
void setup()
{
  ...
  dataFileName = getNextFileName("HARPE_NS141_DATA", ".csv");
  eventFileName = getNextFileName("HARPE_NS141_EVENT", ".txt");
  ...
  if (dataFile)
  {
    dataFile.println(F("time_s,stage,baro_alt_m,baro_pres_pa,baro_temp_c,gps_alt_m,gps_lat_deg,gps_lon_deg,gps_vel_z_m_s,gps_sat_count, bat_volt_v"));
    dataFile.close();
  }
...
void initGPS()
{
  ...
  {
    Serial.println(F(" FAILED"));
      logEvent("GPS initialization FAILED");
      return;
  }
  gpsInitialized = true;
  ...
  if (myGNSS.setDynamicModel(DYN_MODEL_AIRBORNE2g))
    logEvent("GPS dynamic model set: AIRBORNE 2G");
  else
    logEvent("ERROR: Failed to set dynamic model!");
...
void logEvent(String event)
{
  ...
  String timestampStr = getGPSTimeString();
  
  // Log to serial
  Serial.print(F("["));
  if (timestampStr == "NO_FIX") {
    // If GPS time is not available, use millis() as fallback
  ...
  if (sdInitialized)
  {
    ...
    if (eventFile)
    {
      ...
      if (timestampStr == "NO_FIX") {
        ...
        eventFile.print(F("ms,"));
      }
      eventFile.print(timestampStr);
      eventFile.print(F("],"));
...
void readSensors()
{
  ...
  if (baroInitialized)
  {
    ...
    currentData.baroTemp = baro.getTemperature();
  ...
  if (gpsInitialized)
  {
    ...
    // if (!gpsResettle && gpsSettled) {
    //   baselineGPSAlt = gpsMSL;
    //   gpsResettle=true;
    //   logEvent("GPS baseline reset on 2 min settle: " + String(baselineGPSAlt, 2) + " m MSL");
    // }
    if (currentlyHasFix && !gpsHasFix && gpsSettled)
    {
      ...
      if (!gpsEverHadFix)
      {
        ...
        if (!gpsBaselineSet) {
          baselineGPSAlt = gpsMSL;
          gpsBaselineSet = true;
          logEvent("GPS baseline set on first accurate fix: " + String(baselineGPSAlt, 2) + " m MSL");
        }
  ...
  float adcVoltage = (raw / 1023.0) * 3.3;  // Teensy ADC reference
...
void logSensorData()
{
  ...
  dataLine += String(currentData.baroTemp, 2) + ",";
  ...
  dataLine += String(currentData.gpsSatCount, 2) + ",";
  dataLine += String(currentData.batteryVoltage, 2);
...
String getGPSTimeString() {
  ...
  uint32_t msec   = myGNSS.getMillisecond();
